...
const DealPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchDealsAndFoods = async (url: string, setFoodsState: React.Dispatch<React.SetStateAction<Food[]>>, isOwner: boolean = false) => {
      try {
        const dealResponse = await axios.get(url);
        const dealData: DealData[] = dealResponse.data.map((deal: { dealId: number, foodId: number, requesterId: number }) => ({ dealId: deal.dealId, foodId: deal.foodId, requesterId: deal.requesterId }));
        const foodResponses = await Promise.all(dealData.map((deal: DealData) => axios.get(`http://localhost:3000/foods/${deal.foodId}`)));
        const foods = foodResponses.map((res, index) => ({ ...res.data, dealId: dealData[index].dealId }));
      ...
      <div className="flex-grow overflow-y-auto p-4 flex-col justify-center items-center">
        ...
        {myFoodsToShare.map((food, i) => (
          <div className="card card-side flex-grow h-48 w-full bg-base-200 shadow-sm my-1" key={i} style={{ display: 'flex', flexDirection: 'row', height: '100px' }}>
            <figure className="flex-grow" style={{ flex: '1', objectFit: 'cover' }}>
              <img src={food.foodImageUrl} alt={food.foodName} style={{ width: '100%', height: '100px', objectFit: 'cover' }} />
            </figure>
              <div className="card-body flex-grow" style={{ flex: '1', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                <h2 className="card-title text-sm">{food.foodName}</h2>
                <p className="text-sm">Requester: {food.userName}</p>
              </div>
              <div className="card-actions flex-grow" style={{ flex: '1', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                <Link to={`/chat/${food.dealId}`} className="btn btn-primary">Chat</Link>
              </div>
            </div>
          ))}
          ...
          {foodsToShareByOthers.map((food, i) => (
            <div className="card card-side flex-grow h-48 w-full bg-base-200 shadow-sm my-1" key={i} style={{ display: 'flex', flexDirection: 'row', height: '100px' }}>
              <figure className="flex-grow" style={{ flex: '1', objectFit: 'cover' }}>
              <img src={food.foodImageUrl} alt={food.foodName} style={{ width: '100%', height: '100%' , objectFit: 'cover' }} />
              </figure>
              <div className="card-body flex-grow" style={{ flex: '1', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                <h2 className="card-title text-sm">{food.foodName}</h2>
                <p className="text-sm">Owner: <br></br>{food.userName}</p>
              </div>
              <div className="card-actions flex-grow" style={{ flex: '1', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                <Link to={`/chat/${food.dealId}`} className="btn btn-primary">Chat</Link>
              </div>
